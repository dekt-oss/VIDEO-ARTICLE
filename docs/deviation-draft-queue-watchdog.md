...
★ **정확히 말하면 하트비트가 없다.** 워커는 요청을 한꺼번에 집고(기본 5건) 그때 한 번
  `updated_at` 을 찍은 뒤, 각 요청이 **끝날 때**(done/error) 다시 찍는다
  (`engine/draft.py::poll_once`). 처리 **중**에는 아무 신호도 없다. 그래서 뒤쪽 요청은
  앞쪽이 다 끝날 때까지 집을 때의 시각을 달고 있다.
  그래도 60분이 안전한 이유는 **잡 자체가 45분 안에 죽기 때문**이다 — 60분 된 행이 있다는
  것은 그 잡이 이미 없다는 뜻이다. 문턱을 45분 밑으로 줄이려면 **먼저** 요청마다 하트비트를
  찍게 고쳐야 한다. 그 전에 줄이면 살아 있는 워커의 일을 뺏는다.
