// POST /api/render-manage — 렌더 결과 저장/삭제 관리.
// body: { job_id, action: 'save' | 'unsave' | 'trash' | 'restore' | 'purge' }
//   save/unsave  : saved_at 설정/해제(보관 마크)
//   trash/restore: deleted_at 설정/해제(소프트 삭제/복원)
//   purge        : 하드 삭제 — 휴지통(deleted_at 있음)인 잡만, Storage mp4 제거 후 행 삭제(비가역).
//                  비가역이라 운영자 키를 요구한다(/unlock). 파일 삭제는 service_role 로 한다(0045).
...
export async function POST(request: Request) {
  ...
  const { data: job, error: jobErr } = await supabase
    .from("render_jobs")
    .select("id, directive_id, output_url, deleted_at, status")
    .eq("id", body.job_id)
    .maybeSingle();
  ...
  // 소프트 상태 토글(save/unsave/trash/restore)
  ...
  if (action !== "purge") {
    ...
    const { error } = await queueWriter(supabase).from("render_jobs").update(patch).eq("id", job.id);
    if (error) return NextResponse.json({ error: error.message }, { status: 500 });
    return NextResponse.json({ ok: true, action });
  }

  // purge(영구삭제): 반드시 휴지통에 있어야 함 → 실수 방지.
  if (!job.deleted_at) {
  ...
  // Storage mp4 제거(경로 규칙 {directive_id}/{job_id}.mp4 — engine/render.py).
  if (job.output_url) {
    ...
    const { error: rmErr } = await admin.storage.from("renders").remove([`${job.directive_id}/${job.id}.mp4`]);
    if (rmErr) {
      return NextResponse.json(
  ...
  // render_assets 는 directive 공유(재렌더 캐시)라 삭제하지 않는다 — 이 잡 행만 제거.
  const { error } = await queueWriter(supabase).from("render_jobs").delete().eq("id", job.id);
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
