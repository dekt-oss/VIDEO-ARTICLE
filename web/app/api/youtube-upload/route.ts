// POST /api/youtube-upload — ⑥ 렌더 결과에서 완료 mp4 를 유튜브 업로드 큐에 적재.
// body: { job_id, privacy_status? }. lang 은 render_jobs.lang 을 따른다(언어별 채널).
// upload_requests 에 요청 행을 쓰고 publish 워커(GitHub Actions)를 즉시 트리거한다.
...
export async function POST(request: Request) {
  ...
  // 이미 활성(대기/처리중/완료) 업로드가 있으면 재적재하지 않는다(중복 영상 방지 — DB 유니크와 이중 방어).
  ...
  const { error } = await queueWriter(supabase).from("upload_requests").insert({
    render_job_id: job.id,
    paper_id: directive?.paper_id ?? null,
    lang: job.lang ?? "ko",
  ...
  const trig = await triggerPublish();
