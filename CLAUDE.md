...
- **★★★ 실사형 화풍은 2026-09-08 에 확정됐다 — 임의로 바꾸지 말 것.**
  ...
  ▸ **증권 리포트의 "원리"는 논증 단위다**(2026-09-24, 운영자 승인). 리포트에는 논문의 "왜
    ...
    **리포트 지시서 정규화에도 Fact Sheet 전체가 들어간다**(2026-09-25). 전엔 깊이 하나만 넘겨서
    숫자 감사가 빈 원장에 대고 돌았다 — 모든 수치가 빨강(저장 39건에서 378건 → 28건), 지어낸
    출처 카드 연도 "(2024)"도 통과했다. 측정: `python -m scripts.report_factsheet_shadow`(0원).
  ▸ **Jev 배치 확대(2026-09-27, 운영자 "생각보다 잘한다")** — "문장 안에 답이 있는" 질문 넷을 더
    붙였다. 전부 **경고(되묻기)·fail-open**, 차단은 새로 안 만든다. 채점(취향)에는 안 쓴다 —
    사람 낙점을 동전 던지기 수준으로만 맞혔다(`scripts/jev_shadow.py`).
    ① `photo_scene_not_answering` 실사 컷이 나레이션 대신 주제 사진(계약이 약속만 하고 없던 검사)
    ② `photo_number_as_objects` 수치를 사물 개수·높이로("4GW → 엔진 네 대")
    ③ 숫자 감사 빨강 중 단위·표기 환산(631억 ↔ 63.1 billion)은 노랑으로 **내리기만**
    ④ `photo_cause_not_shown` "~해서"의 원인이 이 컷·앞 컷 화면에 없음(계약 ⑨)
    문턱은 전부 그림자 측정에서 정했다(`scripts/{staging,number_objects,number_restated,cause_shown}_shadow.py`).
    지시서 한 장에 1센트 미만.
  ▸ **화면 구성 계약 — "장면 자체가 설명이다"**(2026-09-24, 운영자 지시 "화살표·게이지 같은
    ...
    **4차 렌더(색 개정·주인공 교체 반영, $2.04) 뒤 남은 둘 → 계약 ⑨⑩**: "~해서 ~한다"는 원인
    stage 와 결과 stage 로 **나눈다**(한 장에 넣으면 결과만 남는다) · 도해 부품은 알아볼 물건이어야
    한다(추상 부품 "냉각 채널"이 코일 덩어리가 됐다 → Jev 가 되묻는 경고
    `photo_component_unrecognizable`, fail-open). 주인공이 바뀌는 stage 는 앞 그림을 참조하지
    않는다(`visual_sequence.subject_handoff`) — "세계가 하나"는 같은 장소·재질이지 같은 픽셀이
    아니다. 화풍 문자열을 바꾸면 `PROMPT_CONTRACT_VERSION` 을 **반드시** 올린다(안 올리면 캐시가
    옛 그림을 돌려준다 — 3차 렌더 실측).
